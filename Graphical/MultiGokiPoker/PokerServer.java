...
class PokerServerThread extends Thread{
    ...
    private void sendHandCards(){
        ...
        String[] firstCardNums = new String[PokerServer.PLAYER];
        for(int i = 0; i < PokerServer.PLAYER; ++i){
            firstCardNums[i] = String.valueOf(playerThreads[i].firstCardNum);
        }
        writeMultiStrings(firstCardNums, PokerServer.PLAYER);  //送信番号: S36, 各プレイヤーの初期カード枚数を送信
    }
    //**********CUI(ここまで)**********

    private void readRule(){
        readSingleMessage(); //受信番号: R10, ルール読み終わりを受信
        synchro.synchro();
    }
    ...
    private void reseivePushCard(){
        ...
        writer.println(mainPlayerId);  //送信番号: S12, メインプレイヤーのidを送信
